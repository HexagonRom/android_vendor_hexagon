function __print_hex_functions_help() {
cat <<EOF
...
function eat()
{
    ...
    if (adb shell getprop ro.hex.device | grep -q "$HEXAGON_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.hex.device | grep -q "$HEXAGON_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
