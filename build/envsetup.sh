...
- hexagongerrit:     A Git wrapper that fetches/pushes patch from/to HexagonRom Gerrit Review.
- hexagonrebase:     Rebase a Gerrit change and push it again.
- hexagonremote:      Add git remote for matching Hexagon repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- githubremote:    Add git remote for HexagonRom Github.
...
- repopick:        Utility to fetch changes from HexagonRom Gerrit.
...
function eat()
{
    if [ "$OUT" ] ; then
        ...
        if (adb shell getprop org.hex.device | grep -q "$CUSTOM_BUILD"); then
...
function githubremote()
{
    ...
    local REMOTE=$(git config --get remote.hexagon.projectname)

    local PROJECT=$(echo $REMOTE | sed -e "s#platform/#android/#g; s#/#_#g")

    git remote add github https://github.com/HexagonRom/$PROJECT
...
function installboot()
{
    ...
    if (adb shell getprop org.hex.device | grep -q "$CUSTOM_BUILD");
    then
...
function hexagongerrit() {
    ...
    local user=`git config --get review.gerrit.hexagon.org.username`
...
function hexagonrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "HexagonRom Gerrit Rebase Usage: "
    ...
    git fetch "http://gerrit.hexagon.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop org.hex.device | grep -q "$CUSTOM_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function losrepopick() {
    T=$(gettop)
    $T/vendor/hexagon/build/tools/losrepopick.py $@
}
